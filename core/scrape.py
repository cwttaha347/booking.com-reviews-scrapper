"""
Booking.com Reviews Scraper Core
By Muhammad Taha
"""
...
class Scrape:
    ...
    def _save_local_files(
        ...
        """save local csv and excel files. It creates a directory based on "entity_name" and stores
        the files in it.

        - reviews_{sort_by}.csv: contains reviews in CSV format
        - reviews_{sort_by}.xlsx: contains reviews in Excel format

        Args:
            ls_reviews: list of review objects
        """
        if not ls_reviews:  # Exit if no reviews to save
        ...
        try:
            ...
            if not os.path.exists(self._config.OUTPUT_DIR):
                ...
                self.logger.info(f"Created output directory: {self._config.OUTPUT_DIR}")

            # Create the full directory path for this hotel's reviews
            dir_path = self._LOCAL_OUTPUT_PATH.format(
                output_dir=self._config.OUTPUT_DIR,
                entity_name=self.input_params.hotel_name.replace('/', '_')  # Sanitize filename
            )

            # Create hotel-specific directory if it doesn't exist
            if not os.path.exists(dir_path):
                os.makedirs(dir_path)
                self.logger.info(f"Created hotel directory: {dir_path}")

            # Save CSV
            csv_fname = f"{dir_path}/reviews_{self.input_params.sort_by}.csv"
            try:
                with open(csv_fname, 'w', newline='', encoding='utf-8') as file:
                    if ls_reviews:
                        keys = ['username', 'user_country', 'room_view', 'stay_duration', 
                        ...
                        writer.writeheader()
                        writer.writerows(ls_reviews)
                        self.logger.info(f"Successfully saved CSV file to {csv_fname}")
            except Exception as ex:
                self.logger.error(f"Error saving CSV: {ex}")
                raise

            # Save Excel 
            try:
                excel_fname = f"{dir_path}/reviews_{self.input_params.sort_by}.xlsx"
                df = pd.DataFrame(ls_reviews)
                # Reorder columns to match CSV
                df = df.reindex(columns=keys)
                df.to_excel(excel_fname, index=False, engine='openpyxl')
                self.logger.info(f"Successfully saved Excel file to {excel_fname}")
            except Exception as ex:
                self.logger.error(f"Error saving Excel: {ex}")
                raise

        except Exception as e:
            self.logger.error(f"Error in save_local_files: {str(e)}")
            raise
    ...
    def _get_cond_reviews(self, ls_urls: List[dict]) -> List[dict]:
        ...
        self.logger.info(
            f"Finished Conditional Scraping: {len(ls_reviews)} in {time.time() - _start
